...
def outlook_login_url():
    ...
    auth_url = msal_app.get_authorization_url(
        current_app.config["MS_SCOPE"], redirect_uri=redirect_uri
    )
...
def exchange_code():
    ...
    if not all(
        ...
        return jsonify(
            {"ok": False, "error": "Solicitud inválida o configuración faltante"}
        ), 400
    msal_app = build_msal_app(
        current_app.config["MS_CLIENT_ID"],
        current_app.config["MS_CLIENT_SECRET"],
    ...
    if "access_token" not in result:
        return jsonify(
            {
                "ok": False,
                "error": result.get("error_description", "No se pudo obtener el token"),
            }
        ), 400
