...
def outlook_login_url():
    ...
    if not all([current_app.config["MS_CLIENT_ID"], current_app.config["MS_CLIENT_SECRET"], current_app.config["MS_TENANT_ID"], redirect_uri]):
        return jsonify({"ok": False, "error": "Outlook no está configurado"}), 400
    msal_app = build_msal_app(current_app.config["MS_CLIENT_ID"], current_app.config["MS_CLIENT_SECRET"], current_app.config["MS_TENANT_ID"])
...
def exchange_code():
    ...
    if not all([current_app.config["MS_CLIENT_ID"], current_app.config["MS_CLIENT_SECRET"], current_app.config["MS_TENANT_ID"], code, redirect_uri]):
        return jsonify({"ok": False, "error": "Solicitud inválida o configuración faltante"}), 400
    msal_app = build_msal_app(current_app.config["MS_CLIENT_ID"], current_app.config["MS_CLIENT_SECRET"], current_app.config["MS_TENANT_ID"])
    result = msal_app.acquire_token_by_authorization_code(code, current_app.config["MS_SCOPE"], redirect_uri=redirect_uri)
